...
for($i=0;$i<$length;$i++){
    if($i%2===0){
        ...
        if(!in_array($santalocation, $visitedhouses)){
            array_push($visitedhouses,$santalocation);
        }
    }else{
        if($instructions[$i]==='^'){
            $rx++;
        ...
        if(!in_array($robolocation, $visitedhouses)){
            array_push($visitedhouses,$robolocation);
        }
    } 
}


echo 'Santa and Robo-Santa delivered '.$presentsdelivered.' presents to '.count($visitedhouses).' houses.';
